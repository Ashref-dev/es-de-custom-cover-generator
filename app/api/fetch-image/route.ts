...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      console.error(
        `[API Fetch Image] Failed to fetch image from ${imageUrl}: ${response.status} ${response.statusText}`
      );
      return NextResponse.json(
    ...
    if (!contentType || !contentType.startsWith("image/")) {
      console.error(
        `[API Fetch Image] Fetched content from ${imageUrl} is not an image: ${contentType}`
      );
      return NextResponse.json(
